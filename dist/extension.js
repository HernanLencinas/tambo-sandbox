...
async function updateStatus(vscodeURI) {
    ...
    let workspaceEffectiveStatus = 1;
    try {
        if (sandboxStatus && gitStatus) {
            const workspaceResponse = await sandbox.workspaceStatus();
            if (typeof workspaceResponse === 'number') {
                workspaceEffectiveStatus = workspaceResponse;
            }
            else if (workspaceResponse &&
                typeof workspaceResponse === 'object' &&
                workspaceResponse.data?.estado !== undefined) {
                workspaceEffectiveStatus = workspaceResponse.data.estado;
            }
        }
    }
    catch (error) {
