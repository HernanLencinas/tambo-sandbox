        ...
        webviewView.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                ...
                case 'showMessage':
                    ...
                    break;
                case 'sandboxCreate':
                    const createWorkspaceRes = await vscode.window.showInformationMessage('¿Desplegar un nuevo Workspace en Tambo Sandbox?', { modal: true }, // Modal para enfatizar la confirmación
                    'Sí');
...
async function updateStatus(vscodeURI) {
    ...
    const originalWorkspaceStatus = workspaceStatus.data?.estado;
    ...
    function createStatusHTML(title, status, isCustom = false, errorKey = 'sandbox', showOriginalError = false) {
...
async function updateRepository(workspaceStatus) {
    let html = `
    ...
    const sortedGroups = workspaceStatus.data.repos_disponibles
        ...
        .filter((grupo) => grupo !== null) // Filtrar valores null
        .sort((a, b) => a.toLowerCase().localeCompare(b.toLowerCase())); // Ordenar alfabéticamente ignorando mayúsculas
